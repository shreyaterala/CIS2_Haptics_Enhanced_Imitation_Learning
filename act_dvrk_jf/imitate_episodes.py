...
#   --ckpt_dir ./checkpoints/dvrk_pickup_force6d_planB \
#   --policy_class ACT \
#   --batch_size 8 \
#   --seed 0 \
#   --num_epochs 5000 \
...
def main(args):
    ...
    print(f'[INFO] CF 维度: {CF_DIM} (方案 B: 独立 token,不与 qpos cat)')
    print(f'[INFO] 网络输入: qpos {state_dim} 维 + force {CF_DIM} 维 (各自独立投影)')
...
def eval_bc(config, ckpt_name, save_episode=True):
    ...
    for rollout_id in range(num_rollouts):
        ...
        # ── 方案 B: qpos 和 cf 各自独立保存 ─────────────────────────
        qpos_history = torch.zeros((1, max_timesteps, state_dim)).cuda()
        cf_history   = torch.zeros((1, max_timesteps, cf_dim)).cuda()
        ...
        with torch.inference_mode():
            for t in range(max_timesteps):
                ...
                # ── 方案 B: qpos / cf 分别送进网络,不再 concat ──────
                qpos = torch.from_numpy(qpos_norm).float().cuda().unsqueeze(0)
                cf   = torch.from_numpy(cf_norm).float().cuda().unsqueeze(0)
                qpos_history[:, t] = qpos
                cf_history  [:, t] = cf
                ...
                if config['policy_class'] == 'ACT':
                    if t % query_frequency == 0:
                        all_actions = policy(qpos, cf, curr_image)
                    if temporal_agg:
                    ...
                    raw_action = policy(qpos, cf, curr_image)
                else:
                    raise NotImplementedError
...
def forward_pass(data, policy):
    # ── 方案 B: DataLoader 返回 5 元组 ────────────────────────────
    image_data, qpos_data, cf_data, action_data, is_pad = data
    image_data, qpos_data, cf_data, action_data, is_pad = (
        image_data.cuda(), qpos_data.cuda(), cf_data.cuda(),
        action_data.cuda(), is_pad.cuda())
    return policy(qpos_data, cf_data, image_data, action_data, is_pad)
...
def train_bc(train_dataloader, val_dataloader, config):
    ...
    # ── Sanity check: 第一个 batch 的维度对不对 (方案 B) ─────────
    try:
        first_batch = next(iter(train_dataloader))
        img_b, qpos_b, cf_b, act_b, pad_b = first_batch
        print(f'[sanity] batch shapes:')
        print(f'  image_data:  {tuple(img_b.shape)}')
        print(f'  qpos_data:   {tuple(qpos_b.shape)}  (期望: B, {config["state_dim"]})')
        print(f'  cf_data:     {tuple(cf_b.shape)}    (期望: B, {config["cf_dim"]})')
        print(f'  action_data: {tuple(act_b.shape)}   (期望: B, chunk_size, {config["state_dim"]})')
        print(f'  is_pad:      {tuple(pad_b.shape)}')
        if qpos_b.shape[1] != config["state_dim"]:
            print(f'  ⚠️  qpos_data 维度不对!应该是 {config["state_dim"]}')
        if cf_b.shape[1] != config["cf_dim"]:
            print(f'  ⚠️  cf_data   维度不对!应该是 {config["cf_dim"]}')
